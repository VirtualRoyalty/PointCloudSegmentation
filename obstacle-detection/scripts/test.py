...
def grid_search_optimization(scan, label, obstacle_lst, pipeline, params,
    ...
    try:
        for param in tqdm_notebook(ParameterGrid(params), total=len(
            ...
            if verbose:
                print(
                    'Total time {} ms. Created {} clusters'.format(
            ...
            if score:
                if len(cls_data) > 0 and cls_data['cluster_id'].nunique() > 1:
                    silh_score = silhouette_score(
                        cls_data[['x', 'y', 'z']], cls_data['cluster_id'])
                else:
                    silh_score = 0
                time_exec_dct[json.dumps(param, indent=3)] = (
                    end_time, clusters.shape[0], silh_score)
            else:
                time_exec_dct[json.dumps(param, indent=3)] = (
                    end_time, clusters.shape[0])
        ...
        for scan, label in tqdm_notebook(zip(sorted(scan_lst), sorted(
            ...
            if detailed:
                clusters, cluster_data, stat = pipeline(
                    scan[:, :3], label, obstacle_lst, exec_time=True, **pipeline_params)
                if seg_model:
                    stat['segmentation_time'] = seg_time
                stats.append(stat)
            else:
                clusters, _ = pipeline(
                    scan, label, obstacle_lst, **pipeline_params)
            ...
            if write_path:
                if len(clusters) == 0:
                    np.savetxt(write_path + str(scan_id) +
                               '.bbox', np.empty((0, 0)))
                    if OBB:
                        np.savetxt(write_path + str(scan_id) +
                                   '.segs', np.empty((0, 0)))
                ...
                if write_seg_id:
                    ...
                    for cl_id in sorted(cluster_data['cluster_id'].unique()):
                        seg = cluster_data[cluster_data['cluster_id'] == cl_id].agg(
                            {'seg_id': 'mode'}).values
                        seg_lst.append(seg)
                    seg_arr = np.array(
                        seg_lst, dtype='int64').reshape(
                        1, len(seg_lst))
...
def get_bbox_and_stat_pcl(
    ...
    try:
        for scan, label in tqdm_notebook(zip(sorted(scan_lst), sorted(
            ...
            if detailed:
                clusters, cluster_data, stat = pipeline(
                    scan, label, obstacle_lst, exec_time=True, **pipeline_params)
                stats.append(stat)
            else:
                clusters, _ = pipeline(
                    scan, label, obstacle_lst, **pipeline_params)
            ...
            if write_path:
                ...
                if write_rotated:
                    ...
                    for cluster_id in cluster_data['cluster_id'].unique():
                        tcluster = cluster_data[cluster_data['cluster_id'] == cluster_id][[
                            'x', 'y', 'z']]
                        min_poitns = [
                            tcluster[tcluster.index == indx].values for indx in list(tcluster.idxmin())]
                        max_points = [
                            tcluster[tcluster.index == indx].values for indx in list(tcluster.idxmax())]
